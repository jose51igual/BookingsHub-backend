...
describe('BusinessController', () => {
  ...
  describe('getRecentBookings', () => {
    ...
    it('debería obtener reservas recientes exitosamente', async () => {
      BusinessModel.isBusinessOwner.mockResolvedValue(true);

      await businessController.getRecentBookings(mockRequest, mockResponse);
      ...
      expect(responseCall.message).toBe('Reservas recientes obtenidas exitosamente');
      expect(Array.isArray(responseCall.data)).toBe(true);
    });

    it('debería rechazar usuarios no autorizados', async () => {
      BusinessModel.isBusinessOwner.mockResolvedValue(false);
  ...
  describe('getWeeklyStats', () => {
    ...
    it('debería obtener estadísticas semanales exitosamente', async () => {
      BusinessModel.isBusinessOwner.mockResolvedValue(true);

      await businessController.getWeeklyStats(mockRequest, mockResponse);
      ...
      const responseCall = mockResponse.json.mock.calls[0][0];
      expect(responseCall.success).toBe(true);
      expect(responseCall.message).toBe('Estadísticas semanales obtenidas exitosamente');
      expect(responseCall.data).toHaveProperty('bookings');
      expect(responseCall.data).toHaveProperty('revenue');
      expect(responseCall.data).toHaveProperty('rating');
      expect(responseCall.data).toHaveProperty('period');
    });
